...
export const getPartnerTypes = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Lỗi khi lấy danh sách loại đối tác', error })
  }
}
...
export const getPartnerTypeById = async (req, res) => {
  try {
    ...
    if (!partnerType) {
      return res.status(404).json({ success: false, message: 'Không tìm thấy loại đối tác' })
    }
    res.status(200).json({ success: true, data: partnerType })
  } catch (error) {
    res.status(500).json({ success: false, message: 'Lỗi khi lấy loại đối tác', error })
  }
}
...
export const createPartnerType = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Lỗi khi thêm mới loại đối tác', error })
  }
}
...
export const updatePartnerType = async (req, res) => {
  try {
    ...
    if (!updated) {
      return res.status(404).json({ success: false, message: 'Không tìm thấy loại đối tác để cập nhật' })
    }
    res.status(200).json(updated)
  } catch (error) {
    res.status(500).json({ success: false, message: 'Lỗi khi cập nhật loại đối tác', error })
  }
}
...
export const deletePartnerType = async (req, res) => {
  try {
    ...
    if (!deleted) {
      return res.status(404).json({ success: false, message: 'Không tìm thấy loại đối tác để xóa' })
    }
    res.status(200).json({ success: true, message: 'Xóa thành công' })
  } catch (error) {
    res.status(500).json({ success: false, message: 'Lỗi khi xóa loại đối tác', error })
  }
}
