...
export const createPaymentUrl = async (req, res) => {
    try {
        ...
        const invoice = new Invoice({
            ...
            totalAmount: price, // Tổng tiền cần thanh toán
            discountAmount: 0, // Giả sử chưa có khuyến mãi
            finalAmount: price, // Giá cuối cùng sau giảm giá (nếu có)
        ...
        const vnp_Params = {
            ...
            vnp_Amount: price * 100, // Đơn vị tiền tệ là VND (VNPay yêu cầu phải nhân với 100)
...
export const vnpayIpnHandler = async (req, res) => {
    ...
    try {
        ...
        if (paymentStatus === "Đã thanh toán") {
            ...
            booking.status = "Xác nhận"; // Cập nhật trạng thái Booking thành "Đã xác nhận"
            await booking.save();
        } else {
            // Nếu thanh toán thất bại, cập nhật trạng thái của Booking thành "Đang xử lý" hoặc "Đã hủy"
            const booking = await Booking.findById(invoice.bookingId);
            booking.status = "Đã hủy"; // Hoặc trạng thái khác tùy theo quy định của bạn
            await booking.save();
        }
