...
export const getCancellationBookings = async (req, res) => {
  try {
    const cancellationBookings = await BookingCancellation.find().populate("userId", "username email").populate("bookingId");;
    res.status(200).json({
      success: true,
...
export const updateBookingCancellation = async (req, res) => {
    try {
        ...
        const cancellation = await BookingCancellation.findByIdAndUpdate(id, updatedData, {
            new: true,
            runValidators: true,
        });
        ...
        res.status(200).json({ message: 'Cập nhật yêu cầu hủy thành công.', cancellation });
    } catch (error) {
...
export const deleteBookingCancellation = async (req, res) => {
    try {
        ...
        if (!cancellation) {
            return res.status(404).json({ error: 'Không tìm thấy yêu cầu hủy để xóa.' });
        }

        // Cập nhật lại trạng thái booking nếu cần (tùy yêu cầu logic của bạn)
        // Ví dụ: nếu vẫn muốn cho phép quay lại trạng thái ban đầu:
        await Booking.updateOne({ _id: cancellation.bookingId }, { status: "Chờ xác nhận" });
...
export const confirmMultipleBookingCancellations = async (req, res) => {
  try {
    ...
    if (!Array.isArray(bookingIds) || bookingIds.length === 0) {
      return res.status(400).json({ error: 'Vui lòng chọn ít nhất một phiếu hủy để xác nhận.' })
    }

    // Lấy danh sách phiếu hủy theo bookingId
    const cancellations = await BookingCancellation.find({ bookingId: { $in: bookingIds } })
    ...
    res.status(200).json({ message: 'Xác nhận hoàn tiền cho các phiếu hủy thành công.' })
  } catch (error) {
...
export const rejectMultipleBookingCancellations = async (req, res) => {
  try {
   ...
   if (!Array.isArray(rawIds) || rawIds.length === 0) {
      return res.status(400).json({ error: 'Vui lòng chọn ít nhất một phiếu hủy để từ chối.' });
    }
    const bookingIds = rawIds.map(id => new ObjectId(id));
