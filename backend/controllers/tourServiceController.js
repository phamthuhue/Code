...
export const getAllTourServices = async (req, res) => {
  try {
    const services = await TourService.find().populate("tourId");
...
export const getTourServiceById = async (req, res) => {
  try {
    const service = await TourService.findById(req.params.id)
      .populate("tourId")
      .populate("serviceId");
    if (!service)
...
export const getTourServicesByTourId = async (req, res) => {
  try {
    ...
    if (!serviceData) {
      return res
        .status(404)
        .json({ message: "Không tìm thấy dịch vụ cho tour này." });
    }
    ...
    res
      .status(500)
      .json({
        message: "Lỗi khi lấy dịch vụ theo tourId",
        error: error.message,
      });
  }
};
...
export const createTourServiceForTour = async (req, res) => {
  try {
    ...
    if (existing) {
      return res
        .status(400)
        .json({ message: "TourService cho tour này đã tồn tại." });
    }
    ...
    res
      .status(201)
      .json({ message: "Tạo TourService thành công.", data: newTourService });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Lỗi khi tạo TourService.", error: error.message });
  }
};
...
export const updateTourService = async (req, res) => {
  try {
    const updatedService = await TourService.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
...
export const deleteTourServiceByTourId = async (req, res) => {
  try {
    ...
    const deleted = await TourService.findOneAndDelete({
      tourId: tourObjectId,
    });
    if (!deleted) {
      return res
        .status(404)
        .json({ message: "Không tìm thấy dịch vụ của tour để xóa" });
    }
